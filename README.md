...
To be able to use our code and launch the application, we have to do it in two steps: 
- The first one is the creation of the different tables, which are afterwards used to find a path and make our delay predictions. To do this you have to run the notebook [`data_creation.py`](notebooks/data_creation.py). 
- The second step is to launch the application which is done by rendering the notebook [`app.ipynb`](notebooks/app.ipynb) using Voilà, this can be done by running the `voila app.ipynb` command or selecting the "Render Notebook with Voilà" option in JupyterLab.
